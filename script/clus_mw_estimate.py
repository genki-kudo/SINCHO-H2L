        ...
        for k in temp_dict:
            ...
            if a2num <=(anum/2)+0.5 and dist_cos[k[0]] >= cos_value:
                dist_dict_re[k[0]] = k[1]
                    ...
                    #else:
                    #    dist_dictionary[name+'_'+aname] = int(50000)
                    
                    #print(dist_dictionary[name+'_'+aname])

    return dist_dict_re

def estimate_idealmw(estimate_pocmw, ligand, dist_dictionary):
    extend = {}
        ...
        with open(ligand,'r')as lig:
            for line in lig:
                if line[0:6] =='HETATM' and (i+'_'+line[13:16] in dist_dictionary.keys()):
                    ...
                    ideal_mw = estimate_pocmw[i]+(12*float(dist_dictionary[i+'_'+line[13:16]])/1.53)-12
...
def extend_startatom(extend, ideal_mw):
    ...
    print(best)
    keys1 = [k for k, v in extend.items() if v == best]

    extend_1 = extend.pop(keys1[0])
    mw_all = list(extend.values())
    best = getNearestValue(mw_all, ideal_mw)
    print(best)
    keys2 = [k for k, v in extend.items() if v == best]

    extend_2 = extend.pop(keys2[0])
    mw_all = list(extend.values())
    best = getNearestValue(mw_all, ideal_mw)
    print(best)
    keys3 = [k for k, v in extend.items() if v == best]
