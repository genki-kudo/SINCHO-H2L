...
def dict_pdb_noh(pdbfile):
    ...
    for line in open(pdbfile).readlines():
        if (line[0:6]=="HETATM" or line[0:6]=="ATOM  ") and line.split()[-1]!='H':
...
def dict_pdb_noh_nohetatm(pdbfile):
    dict = {}
    for line in open(pdbfile).readlines():
        if line[0:6]=="ATOM  " and line.split()[-1]!='H':
            dict[str(line[6:11])] = np.array([float(line[30:38]), float(line[38:46]), float(line[46:54])])
    return dict
...
def logo():
    ...
    print('|====================================================================|')
    print('|                                                                    |')
    print('|   SSSSSS   IIII  NN    NN   CCCCCCC  HH    HH   OOOOOO             |')
    ...
    print('|====================================================================|')
    print('')
    print('|=======    Developed by Genki Kudo, Univ. Tsukuba  (2023)    =======|')
    print('')
    print('')
